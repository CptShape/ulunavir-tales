...
`).map(g=>g.replace(/^- /,"").trim()).map(g=>`<li>${g}</li>`).join("")}</ul>`).split(/\n{2,}/).map(u=>/^<(h\d|ul|pre|p)/.test(u.trim())?u:`<p>${u.replace(/\n/g,"<br />")}</p>`).join("")}function ft(e){return new Intl.DateTimeFormat("en",{dateStyle:"medium",timeStyle:"short"}).format(new Date(e))}function Ie(e,t,a){if(!t)return e;const r=t.toLowerCase();return e.filter(s=>a(s).toLowerCase().includes(r))}function $e(e){return[...new Set(e.flatMap(t=>t.tags))].sort((t,a)=>t.localeCompare(a))}function ke(e=""){return`
  ...
  `,(s||n||i)&&ht.querySelector(".content").insertAdjacentHTML("afterbegin",`${i}${n}${s}`)}async function Ae(){const e=I();if(!e)return G("Sign in to manage account settings.");H(`
      <div class="stack">
    ...
    `,"home")}function yt(e,t,a){return`<a class="nav-link ${a?"is-active":""}" href="#${e}"><span>${t}</span></a>`}function Ee(){return`
    <section class="hero">
      <div class="stack">
  ...
  `:""}async function Te(){const e=I();let t=[];if(e?.email)try{t=await o.adapter.listIncomingStoryTransfers?.(e.email)??[]}catch(a){console.error("Incoming transfer list failed:",a),o.loadError="Ownership requests could not be loaded right now."}H(`
    ...
    `,"home")}async function Ce(){const e=I(),t=await o.adapter.listCreatorStories(e?.id);let a=[];if(e?.email)try{a=await o.adapter.listIncomingStoryTransfers?.(e.email)??[]}catch(u){console.error("Incoming transfer list failed:",u),o.loadError="Ownership requests could not be loaded right now."}const r=x(),s=r.get("q")??"",n=r.get("tag")??"",i=Ie(t,s,u=>`${u.title} ${u.tags.join(" ")}`).filter(u=>n?u.tags.includes(n):!0),c=$e(t),d=o.authClient?.mode==="firebase"&&!e?'<div class="notice">Sign in with Firebase to create, edit, and manage your own stories.</div>':"";H(`
      <div class="stack">
        <div class="page-title">
          <div>
            <h2>Creator</h2>
        ...
        <section class="story-list">
          ${i.length?i.map(Pe).join(""):'<div class="empty-state">No stories match this filter yet.</div>'}
        </section>
      </div>
    `,"creator")}function Pe(e){return`
    <article class="list-card">
  ...
  `}async function Oe(){const e=await o.adapter.listBrowserStories(I()?.id),t=x(),a=t.get("group")!=="flat",r=t.get("creator")??"",s=r?e.filter(c=>c.creatorName===r):e,n=[...new Set(e.map(c=>c.creatorName))];let i="";s.length?a?i=n.filter(c=>!r||c===r).map(c=>{const d=s.filter(u=>u.creatorName===c);return d.length?`
          <section class="panel stack">
            <div class="section-header">
              <h3>${l(c)}</h3>
  ...
  `}async function Ne(e){const t=await o.adapter.getStory(e);if(!t)return G("Story not found.");const a=$t(t),r=x().get("view")==="browser",s=xt(),n=x().get("transfer")==="1",i=t.pendingTransferStatus==="pending"?t.pendingTransfer:null;if(t.visibility==="private"&&!a)return G("This story is private.");H(`
      <div class="stack">
        ...
        <section class="nested-list ${s==="list"?"is-list-view":""}">
          ${t.arcs.length?t.arcs.map((c,d)=>Le(c,t,a,d,r)).join(""):'<div class="empty-state">No arcs yet. Create the first arc to start structuring this story.</div>'}
        </section>
      </div>
    `,r?"browser":a?"creator":"browser")}function Le(e,t,a,r,s=!1){return`
    <article class="list-card">
      <div class="split-header">
        <div>
          <h3>${l(e.title)}</h3>
  ...
  `}function De(e){const t=e.soundtracks??[];return`
    <section class="panel stack">
      <div class="section-header">
        <div>
          <h3>Soundtracks</h3>
  ...
  `}function Re(e){if(!e.length)return"";const t=z(),a=R(o.soundtrack.volume);return`
  ...
  `}async function Ue(e,t){const[a,r]=await Promise.all([o.adapter.getStory(e),o.adapter.getArc(t)]);if(!a||!r)return G("Arc not found.");const s=$t(a),n=x().get("view")==="browser",i=xt();if(a.visibility==="private"&&!s)return G("This story is private.");const c=(r.phases??[]).map(d=>`
    <section class="phase-block stack">
      ${qe(d,s,n,r.id)}
      <div class="nested-list ${i==="list"?"is-list-view":""}">
        ${d.chapters.length?d.chapters.map((u,p)=>xe(u,a,r,s,p,n,d)).join(""):'<div class="empty-state">No chapters in this phase yet.</div>'}
      </div>
    </section>
  `).join("");if(H(`
      <div class="stack">
    ...
    `,n?"browser":s?"creator":"browser"),s&&!n){const d=document.querySelector("#story-transfer-button");d&&d.addEventListener("click",u=>{u.preventDefault(),u.stopPropagation(),showStoryTransferModal(a.id)})}}function xe(e,t,a,r,s,n=!1,i=null){return`
    <article class="list-card">
      <div class="split-header">
        <div>
  ...
  `}async function _e(e,t,a){const[r,s,n]=await Promise.all([o.adapter.getStory(e),o.adapter.getArc(t),o.adapter.getChapter(a)]);if(!r||!s||!n)return G("Chapter not found.");const i=$t(r),c=x().get("view")==="browser";if(r.visibility==="private"&&!i)return G("This story is private.");const d=n.assets??[],u=c?me(n.soundtracks??[]):[],p=(s.chapters??[]).findIndex(_=>_.id===a),g=p>0?s.chapters[p-1]:null,A=p>=0&&p<s.chapters.length-1?s.chapters[p+1]:null,N=Pt(r.id,s.id,g,A,c),L=Pt(r.id,s.id,g,A,c),C=i&&!c?`
    ...
    `,c?"browser":i?"creator":"browser",Re(u)),c&&u.length?be(n.id,u):V()}function Ot(e){const t=e.url??e.dataUrl??"";return`
  ...
  `}function G(e){H(`
    ...
    `,"home")}function kt(e){return`<div class="breadcrumbs">${e.map(([t,a])=>t?`<a href="${t}">${l(a)}</a>`:`<span>${l(a)}</span>`).join("<span>/</span>")}</div>`}async function f(){switch(ue(),o.loadError="",o.route=le(),o.route.name){case"home":return V(),Te();case"creator":return V(),Ce();case"browser":return V(),Oe();case"settings":return V(),Ae();case"story":return V(),Ne(o.route.params.storyId);case"arc":return V(),Ue(o.route.params.storyId,o.route.params.arcId);case"chapter":return _e(o.route.params.storyId,o.route.params.arcId,o.route.params.chapterId);default:return V(),G("This page does not exist.")}}async function at(){try{await f()}catch(e){console.error("Render failed:",e),o.loadError=String(e?.message||e||"The page could not be rendered."),ht.innerHTML=`
      <main class="content">
        <section class="panel stack">
