...
`).map(v=>v.replace(/^- /,"").trim()).map(v=>`<li>${v}</li>`).join("")}</ul>`).split(/\n{2,}/).map(l=>/^<(h\d|ul|pre|p)/.test(l.trim())?l:`<p>${l.replace(/\n/g,"<br />")}</p>`).join("")}function rt(a){return new Intl.DateTimeFormat("en",{dateStyle:"medium",timeStyle:"short"}).format(new Date(a))}function _t(a,t,e){if(!t)return a;const s=t.toLowerCase();return a.filter(r=>e(r).toLowerCase().includes(s))}function Lt(a){return[...new Set(a.flatMap(t=>t.tags))].sort((t,e)=>t.localeCompare(e))}function D(a,t){const e=I(),s=d.authError?`<div class="notice"><strong>Sign-in error</strong><div class="muted">${u(d.authError)}</div></div>`:"";et.innerHTML=`
      ...
      <aside class="sidebar">
        ...
        <div class="stack">
          <button class="notice account-card" data-action="open-settings" ${e?"":"disabled"}>
            ...
            <div class="muted">${u(e?.email??(d.authClient?.mode==="firebase"?"Sign in to create and manage stories":"Local demo mode"))}</div>
          </button>
          <button class="login-button" data-action="toggle-login">
            ${d.currentUser?"Log out":"Log in"}
          </button>
        </div>
      </aside>
  ...
  `}async function Bt(){D(`
    ...
    `,"home")}async function qt(){const a=I(),t=await d.adapter.listCreatorStories(a?.id),e=B(),s=e.get("q")??"",r=e.get("tag")??"",i=_t(t,s,c=>`${c.title} ${c.tags.join(" ")}`).filter(c=>r?c.tags.includes(r):!0),n=Lt(t),o=d.authClient?.mode==="firebase"&&!a?'<div class="notice">Sign in with Firebase to create, edit, and manage your own stories.</div>':"";D(`
      <div class="stack">
        <div class="page-title">
          <div>
            <h2>Creator</h2>
        ...
        <section class="panel stack">
          ...
          <div class="chip-row">
            ${n.map(c=>`<a class="pill" href="#/creator?tag=${encodeURIComponent(c)}">${u(c)}</a>`).join("")}
          </div>
        </section>
  ...
  `}async function Mt(){const a=await d.adapter.listBrowserStories(I()?.id),t=B(),e=t.get("group")!=="flat",s=t.get("creator")??"",r=s?a.filter(o=>o.creatorName===s):a,i=[...new Set(a.map(o=>o.creatorName))];let n="";r.length?e?n=i.filter(o=>!s||o===s).map(o=>{const c=r.filter(l=>l.creatorName===o);return c.length?`
          <section class="panel stack">
            <div class="section-header">
              <h3>${u(o)}</h3>
              <span class="pill">${c.length} public stories</span>
            </div>
            <div class="story-list">${c.map(ot).join("")}</div>
          </section>
  ...
  `}async function Vt(a){const t=await d.adapter.getStory(a);if(!t)return O("Story not found.");const e=st(t),s=B().get("view")==="browser",r=mt();if(t.visibility==="private"&&!e)return O("This story is private.");D(`
      <div class="stack">
  ...
  `}async function Jt(a,t){const[e,s]=await Promise.all([d.adapter.getStory(a),d.adapter.getArc(t)]);if(!e||!s)return O("Arc not found.");const r=st(e),i=B().get("view")==="browser",n=mt();if(e.visibility==="private"&&!r)return O("This story is private.");const o=(s.phases??[]).map(c=>`
    <section class="phase-block stack">
      ${Gt(c,r,i,s.id)}
      <div class="nested-list ${n==="list"?"is-list-view":""}">
        ${c.chapters.length?c.chapters.map((l,h)=>Ht(l,e,s,r,h,i,c)).join(""):'<div class="empty-state">No chapters in this phase yet.</div>'}
      </div>
    </section>
  `).join("");D(`
      <div class="stack">
  ...
  `}async function Yt(a,t,e){const[s,r,i]=await Promise.all([d.adapter.getStory(a),d.adapter.getArc(t),d.adapter.getChapter(e)]);if(!s||!r||!i)return O("Chapter not found.");const n=st(s),o=B().get("view")==="browser";if(s.visibility==="private"&&!n)return O("This story is private.");const c=i.assets??[],l=(r.chapters??[]).findIndex(ft=>ft.id===e),h=l>0?r.chapters[l-1]:null,v=l>=0&&l<r.chapters.length-1?r.chapters[l+1]:null,N=Kt(s.id,r.id,h,v,o),J=n&&!o?`
          ...
          <section class="editor-pane">
            <div class="editor-controls">
              ...
              <div class="asset-list">
                ${c.length?c.map(ct).join(""):'<div class="empty-state">No assets in this chapter yet.</div>'}
              </div>
              <div class="notice mono">${u(d.saveStatus||"Tip: use `![alt](image-url)` to place pasted external images into the chapter body.")}</div>
            </div>
          </section>
      ...
      `:`
        <section class="panel stack">
          <div class="section-header">
            <h3>Reading view</h3>
            <span class="pill">${c.length} asset(s)</span>
          </div>
          <div class="markdown-preview">${at(i.body||"*This chapter is empty.*")}</div>
        </section>
        ${c.length?`<section class="panel stack"><h3>Referenced images</h3><div class="asset-list">${c.map(ct).join("")}</div></section>`:""}
      `;D(`
      <div class="stack">
  ...
  `}function O(a){D(`
    ...
    `,"home")}function it(a){return`<div class="breadcrumbs">${a.map(([t,e])=>t?`<a href="${t}">${u(e)}</a>`:`<span>${u(e)}</span>`).join("<span>/</span>")}</div>`}async function f(){switch(d.route=kt(),d.route.name){case"home":return Bt();case"creator":return qt();case"browser":return Mt();case"settings":return Ft();case"story":return Vt(d.route.params.storyId);case"arc":return Jt(d.route.params.storyId,d.route.params.arcId);case"chapter":return Yt(d.route.params.storyId,d.route.params.arcId,d.route.params.chapterId);default:return O("This page does not exist.")}}function Zt(){return{title:document.querySelector("#story-title-input")?.value.trim()??"",tags:(document.querySelector("#story-tags-input")?.value??"").split(",").map(a=>a.trim()).filter(Boolean),visibility:document.querySelector("#story-visibility-input")?.value??"private"}}function dt(a,t,e){const s=[...a],[r]=s.splice(t,1);return s.splice(e,0,r),s}async function lt(){if(d.currentUser)return await d.authClient.signOut(),C(null),d.saveStatus="Signed out.",d.authError="",f();if(d.authClient.mode==="firebase")try{const t=await d.authClient.signIn();return await d.adapter.seedDemoStory?.({id:t.uid,name:t.displayName||t.email||"Creator",email:t.email}),C({id:t.uid,name:t.displayName||t.email||"Creator",email:t.email,mode:"firebase",structureView:"list"}),d.authError="",d.saveStatus="Signed in with Firebase.",f()}catch(t){return console.error("Firebase sign-in failed:",t),d.saveStatus="",d.authError=Wt(t),f()}const a=document.createElement("div");a.className="modal-backdrop",a.innerHTML=`
  ...
  `,document.body.append(a),a.querySelector("#modal-login-cancel").addEventListener("click",()=>a.remove()),a.querySelector("#modal-login-submit").addEventListener("click",()=>{const t=a.querySelector("#login-name").value.trim()||"Creator",e=a.querySelector("#login-email").value.trim()||"local@storyforge.local";C({id:`local-${t.toLowerCase().replaceAll(/\s+/g,"-")}`,name:t,email:e,mode:"local",structureView:"list"}),a.remove(),d.saveStatus="Signed in with a local demo profile.",d.authError="",f()})}function Wt(a){const t=a?.code?String(a.code):"",e=a?.message?String(a.message):"Unknown sign-in error.";return t==="auth/unauthorized-domain"?"This site domain is not authorized in Firebase Auth. Add your local/dev domain and your GitHub Pages domain in Firebase Console > Authentication > Settings > Authorized domains.":t==="auth/popup-closed-by-user"?"The sign-in popup closed before Firebase completed the login. If it closes instantly every time, double-check Authorized domains and the Google sign-in provider setup.":t==="auth/operation-not-allowed"?"Google sign-in is not enabled for this Firebase project. Enable it in Firebase Console > Authentication > Sign-in method.":t==="auth/invalid-api-key"?"Your Firebase API key is invalid. Recheck the values in your `.env` file and restart the dev server.":t==="auth/network-request-failed"?"Firebase could not complete the sign-in request. Check your connection and any browser privacy extensions blocking popups or auth requests.":t?`${t}: ${e}`:e}async function Qt(a){const t=d.route.params.chapterId,e=await d.adapter.getChapter(t);if(!e)return;const s=[...e.assets??[]];for(const n of a){const o=await ee(n);s.push({id:crypto.randomUUID(),name:n.name,type:n.type,size:n.size,dataUrl:o})}const r=document.querySelector("#chapter-body-input"),i=s.slice((e.assets??[]).length).map(n=>`
![${n.name}](${n.dataUrl})`).join("");await d.adapter.updateChapter(t,{assets:s,body:`${r.value}${i}`}),d.dragActive=!1,d.saveStatus="Assets added to the chapter. In production these should upload to object storage instead of local state.",await f()}function Xt(a){const t=a.trim();if(!t)throw new Error("Add an image URL first.");let e;try{e=new URL(t)}catch{throw new Error("That image URL is not valid.")}if(!["http:","https:"].includes(e.protocol))throw new Error("Use an http or https image URL.");return e.toString()}async function te(a){const t=await d.adapter.getChapter(a);if(!t)throw new Error("Chapter not found.");const e=document.querySelector("#asset-name-input"),s=document.querySelector("#asset-url-input"),r=document.querySelector("#chapter-body-input"),i=e?.value.trim()||"image",n=Xt(s?.value??""),o={id:crypto.randomUUID(),name:i,type:"image/external",url:n},c=[...t.assets??[],o],l=`${r?.value??t.body??""}
![${i}](${n})`;await d.adapter.updateChapter(a,{assets:c,body:l}),d.saveStatus="External image link added and markdown updated.",await f()}async function pt(){const a=I();if(!a?.id)return;const t=await d.adapter.getUserProfile?.(a.id);t&&C({...a,name:t.name??a.name,email:t.email??a.email,penName:t.penName??"",structureView:t.structureView??a.structureView??"list"})}function X(a){return window.confirm(`Are you sure you want to delete this ${a}? This cannot be undone.`)}function ee(a){return new Promise((t,e)=>{const s=new FileReader;s.onload=()=>t(String(s.result)),s.onerror=()=>e(s.error),s.readAsDataURL(a)})}document.addEventListener("click",async a=>{const t=a.target.closest("[data-action]");if(!t)return;const e=t.dataset.action;if(e==="toggle-login")return lt();if(e==="open-settings")return $("/settings");if(e==="set-structure-view"){const s=I(),r=t.dataset.view==="list"?"list":"grid";if(!s?.id)return C({...s,structureView:r}),f();const i=await d.adapter.updateUserProfile(s.id,{name:s.name,email:s.email,penName:s.penName??"",structureView:r});return C({...s,structureView:i.structureView??r,penName:i.penName??s.penName??"",name:i.name??s.name,email:i.email??s.email}),f()}if(e==="apply-story-filters"){const s=document.querySelector("#story-search").value.trim(),r=document.querySelector("#story-tag-filter").value;return $(`/creator${s||r?`?${new URLSearchParams({q:s,tag:r}).toString()}`:""}`)}if(e==="apply-browser-filters"){const s=document.querySelector("#browser-creator-filter").value,r=document.querySelector("#browser-group-mode").value;return $(`/browser?${new URLSearchParams({creator:s,group:r}).toString()}`)}if(e==="create-story"){const s=I();if(!s)return d.saveStatus="Sign in first to create stories in Firebase mode.",lt();const r=await d.adapter.createStory({creatorId:s.id,creatorName:ht(s),title:"Untitled Story",tags:["draft"],visibility:"private"});return $(`/stories/${r.id}`)}if(e==="save-story-settings"){const s=t.dataset.storyId,r=Zt();return await d.adapter.updateStory(s,r),d.saveStatus="Story details saved.",f()}if(e==="create-arc"){const s=t.dataset.storyId,r=await d.adapter.createArc(s,`Arc ${Math.floor(Math.random()*90+10)}`);return $(`/stories/${s}/arcs/${r.id}`)}if(e==="save-arc-title")return await d.adapter.updateArc(t.dataset.arcId,{title:document.querySelector("#arc-title-input").value.trim()||"Untitled Arc"}),d.saveStatus="Arc title saved.",f();if(e==="move-arc-up"||e==="move-arc-down"){const s=await d.adapter.getStory(t.dataset.storyId),r=Number(t.dataset.index),i=e==="move-arc-up"?-1:1;return await d.adapter.reorderArcs(s.id,dt(s.arcIds,r,r+i)),f()}if(e==="create-chapter"){const s=await d.adapter.createChapter(t.dataset.arcId,"Untitled Chapter");return $(`/stories/${t.dataset.storyId}/arcs/${t.dataset.arcId}/chapters/${s.id}`)}if(e==="create-phase"){const s=window.prompt("Phase title","New Phase");return s===null?void 0:(await d.adapter.createPhase(t.dataset.arcId,s),d.saveStatus="Phase created.",f())}if(e==="rename-phase"){const s=window.prompt("Rename phase",t.dataset.phaseTitle||"Phase");return s===null?void 0:(await d.adapter.renamePhase(t.dataset.arcId,t.dataset.phaseId,s),d.saveStatus="Phase renamed.",f())}if(e==="move-chapter-up"||e==="move-chapter-down"){const s=await d.adapter.getArc(t.dataset.arcId),r=(s.phases??[]).find(o=>o.id===t.dataset.phaseId);if(!r)return;const i=Number(t.dataset.index),n=e==="move-chapter-up"?-1:1;return await d.adapter.reorderPhaseChapters(s.id,r.id,dt(r.chapterIds,i,i+n)),f()}if(e==="save-chapter"){const s=t.dataset.chapterId;return await d.adapter.updateChapter(s,{title:document.querySelector("#chapter-title-input").value.trim()||"Untitled Chapter",body:document.querySelector("#chapter-body-input").value}),d.saveStatus="Chapter saved.",f()}if(e==="save-pen-name"){const s=I(),r=document.querySelector("#pen-name-input").value.trim(),i=await d.adapter.updateUserProfile(s.id,{name:s.name,email:s.email,penName:r});return C({...s,penName:i.penName??"",name:i.name??s.name,email:i.email??s.email}),d.saveStatus=r?"Pen name saved.":"Pen name cleared. Account name will be used.",f()}if(e==="delete-story")return X("story")?(await d.adapter.deleteStory(t.dataset.storyId),d.saveStatus="Story deleted.",$("/creator")):void 0;if(e==="delete-arc")return X("arc")?(await d.adapter.deleteArc(t.dataset.arcId),d.saveStatus="Arc deleted.",$(`/stories/${t.dataset.storyId}`)):void 0;if(e==="delete-chapter")return X("chapter")?(await d.adapter.deleteChapter(t.dataset.chapterId),d.saveStatus="Chapter deleted.",$(`/stories/${t.dataset.storyId}/arcs/${t.dataset.arcId}`)):void 0;if(e==="add-external-asset")try{return await te(t.dataset.chapterId)}catch(s){return d.saveStatus=String(s.message||s),f()}});document.addEventListener("change",async a=>{const t=a.target;if(t instanceof HTMLSelectElement&&t.dataset.action==="move-chapter-phase")return await d.adapter.moveChapterToPhase(t.dataset.arcId,t.dataset.chapterId,t.value),d.saveStatus="Chapter moved to another phase.",f()});document.addEventListener("input",a=>{if(a.target.id==="chapter-body-input"){const t=document.querySelector(".markdown-preview");t&&(t.innerHTML=at(a.target.value||"*Start writing to preview your chapter here.*"))}if(a.target.id==="chapter-title-input"){const t=a.target.value.trim()||"Untitled chapter",e=document.querySelector(".page-title h2");e&&(e.textContent=t)}});document.addEventListener("dragover",a=>{if(d.route.name!=="chapter")return;a.preventDefault(),d.dragActive=!0;const t=document.querySelector("[data-dropzone='assets']");t&&t.classList.add("is-active")});document.addEventListener("dragleave",a=>{if(d.route.name!=="chapter"||a.relatedTarget)return;d.dragActive=!1;const t=document.querySelector("[data-dropzone='assets']");t&&t.classList.remove("is-active")});document.addEventListener("drop",async a=>{if(d.route.name!=="chapter")return;a.preventDefault();const t=document.querySelector("[data-dropzone='assets']");t&&t.classList.remove("is-active");const e=[...a.dataTransfer.files].filter(s=>s.type.startsWith("image/"));e.length&&await Qt(e)});window.addEventListener("hashchange",()=>{d.saveStatus="",f()});async function ae(){const a=Ut();d.authClient=a,d.adapter=await Dt(a),d.authClient.mode==="firebase"?d.authClient.watchAuth(t=>{t?(C({id:t.uid,name:t.displayName||t.email||"Creator",email:t.email,mode:"firebase"}),pt().finally(()=>f())):(C(null),f())}):d.currentUser?.id&&await pt(),window.location.hash?f():$("/")}ae().catch(a=>{et.innerHTML=`
    <main class="content">
      <section class="panel">
